...
└── Generate Typst code (technical details: typst skill)
      ↓
...
**Slide skill 自身需要委托的能力**（这些是 slide skill 没有的能力，需要其他 agent 补充）：
...
当使用 `genimg` skill 生成多张图片时，genimg 本身是 one-shot 工作，无法保持风格统一。此时需要 slide skill 自己管理风格一致性：
...
详细检查流程见 `typst` skill 的 `references/touying.md` → "Page Validation" 章节。
...
[deep-research skill] → Gather new information from web
         ↓
[wide-research skill] → Process existing materials in batch
         ↓
[slide skill] → Narrative design + content transformation
         ↓
...
2. **If missing or empty**: Invoke research skills first
   ...
   - `wide-research` → for processing existing local materials
